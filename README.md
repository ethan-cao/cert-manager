...
This folder is generated by helm, this downoads the packaged yaml files
```
helm pull --untar jetstack/cert-manager
```

To install the chart with the release name `cert-manager` for version `v1.17.2`
...
helm install cert-manager ./cert-manager \
  ...
  --set cainjector.image.tag=$VERSION \
  --set acmesolver.image.tag=$VERSION \
  --set startupapicheck.image.tag=$VERSION
```
